...
function bot(torrent, bot) {
  bot.onText(/\/start/, async msg => {
    bot.sendMessage(msg.-1001328512229, startMessage);
  });

  bot.on("message", async msg => {
    if (!msg.document) return;
    const chatId = -1001328512229;
  ...
  bot.onText(detailsRegex, async (msg, match) => {
    ...
    bot.sendMessage(-1001328512229, "Loading...");

    const data = await axios(`${api}/details/${site}?query=${query}`).then(({ data }) => data);
    if (!data || data.error) {
      bot.sendMessage(-1001328512229, "An error occured");
      ...
      await bot.sendMessage(-1001328512229-1001328512229, result1);
      await bot.sendMessage(-1001328512229, result2);
      await bot.sendMessage(-1001328512229, torrent.downloadLink);
    }
  });
  ...
  bot.onText(downloadRegex, (msg, match) => {
    ...
    const reply = async torr => {
      ...
      try {
        ...
        } else messageObj = await bot.sendMessage(-1001328512229, mess1);
      } catch (e) {
        console.log(e.message);
      }
    };

    const onDriveUpload = (torr, url) => bot.sendMessage(-1001328512229, `${torr.name} uploaded to drive\n${url}`);
    const onDriveUploadStart = torr => bot.sendMessage(-1001328512229, `Uploading ${torr.name} to gdrive`);

    if (link.indexOf("magnet:") !== 0) {
      bot.sendMessage(-1001328512229, "Link is not a magnet link");
    } else {
      bot.sendMessage(-1001328512229, "Starting download...");
      try {
  ...
  bot.onText(removeRegex, (msg, match) => {
    ...
    try {
      torrent.remove(link);
      bot.sendMessage(-1001328512229, "Removed");
    } catch (e) {
      bot.sendMessage(-1001328512229, `${e.message}`);
    }
  });
}
